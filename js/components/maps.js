var Maps = {
    ...
    fixContentHeight: function() {
        var footer = $("#mappage > div[data-role='footer']"),
            content = $("#mappage > div[data-role='content']"),
    ...
    init: function(onSelectCB) {
        ...
        var marker = this.getMarker(2013);
        markerLayer.addFeatures(marker);*/

        // create map
        Maps.map = new OpenLayers.Map({div: "map", theme: null});
        /*Maps.map.addControls([
            new OpenLayers.Control.Attribution(),
            new OpenLayers.Control.SelectFeature(markerLayer, {
                autoActivate:true,
                onSelect: onSelectCB
            })
        ]);*/
        Maps.map.addLayers( new OpenLayers.Layer.OSM() );
        Maps.map.zoomToMaxExtent();
        Maps.map.updateSize();
    }
};